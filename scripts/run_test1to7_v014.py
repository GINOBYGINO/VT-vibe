"""v0.14 regression for test1~7: from-step 5 (word-level subtitles + silero VAD).

Main delta vs v0.13:
  - Word-level subtitle event timing (WhisperX / faster-whisper words)
  - Break priority: silence gap → punct → jieba → char limit
  - MIN_SUB_SEC / LINGER_SEC / soft clamp (no silent drops)
  - Per-clip silero VAD for subtitle timing guards

Outputs land under outputs/v0.14/<alias>/.
test5 additionally writes AB subtitles under:
  outputs/v0.14/test5/fast/
  outputs/v0.14/test5/whisperx/
"""
...
# Force version folder (do not setdefault — parent shell may still have v0.13).
os.environ["OUTPUT_VERSION"] = "v0.14"
...
def _has_nosub(job_dir: Path) -> bool:
    edit = job_dir / "04_edit"
    if not edit.is_dir():
        return False
    return any(edit.glob("short_*_nosub.mp4"))
...
def main() -> None:
    ...
    for alias, substr in TARGETS:
        ...
        try:
            out_alias = ROOT / "outputs" / "v0.14" / alias
            existing = []
            if alias == "test5":
            ...
            if job is None or not _has_highlights(job):
            ...
            elif not _has_nosub(job):
                print(f"[{alias}] nosub missing → from-step 4: {job}", flush=True)
                run_pipeline(
                    job_dir=job,
                    from_step=4,
                    whisper_model="small",
                ...
                # v0.14 only changes subtitle layer → reuse edit artifacts.
                print(f"[{alias}] rerun from-step 5: {job}", flush=True)
                run_pipeline(
                    job_dir=job,
                    from_step=5,
                    whisper_model="small",
    ...
    out = ROOT / "outputs" / "v0.14" / "test1to7_summary.json"
