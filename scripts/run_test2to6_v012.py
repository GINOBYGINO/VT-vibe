"""v0.12 regression for test2~6: from-step 3 --auto-arcs + WhisperX subtitles.

Outputs land under outputs/v0.12/<alias>/.
test5 additionally writes AB subtitles under:
  outputs/v0.12/test5/fast/
  outputs/v0.12/test5/whisperx/

Does not re-run full-video ASR (step 2); re-selects arcs and reburns subtitles.
"""
...
os.environ.setdefault("OUTPUT_VERSION", "v0.12")
...
def main() -> None:
    ...
    for alias, substr in TARGETS:
        ...
        try:
            ...
            print(f"[{alias}] rerun from-step 3 --auto-arcs: {job}", flush=True)
            run_pipeline(
                job_dir=job,
                from_step=3,
                whisper_model="small",
            ...
            out_alias = ROOT / "outputs" / "v0.12" / alias
    ...
    out = ROOT / "outputs" / "v0.12" / "test2to6_summary.json"
